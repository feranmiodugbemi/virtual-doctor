...
def en_report():
    if not session.get("name") or session.get("language") != 'english':
        return redirect("/")
    return render_template("en/8-report.html", disease1=session.get("disease1"), disease2=session.get("disease2"), disease3=session.get("disease3"))
...
#Only route for now, that might need conversion from igbo to english 
